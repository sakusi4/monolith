...
1. 테스트는 두 종류만 쓴다. 기준 구현 Miniflux와 같은 방식이다.
   ...
   - **HTTP 통합 테스트**: `cmd/server/{기능}_test.go`에서 `routes()` 전체에 실제 DB로 `httptest` 요청(폼 POST 포함)을 보내 기능의 사용자 흐름을 따라간다. 상태 코드, 리다이렉트 위치, HTML 본문에 기대한 값이 있는지 확인한다(9.15).
...
14. 유지 비용이 검증 가치보다 큰 테스트는 쓰지 않는다. 유지 비용은 무해한 변경(문구, 마크업, 순서, struct 모양)에도 고쳐야 하는 수고, 픽스처, DB 왕복, 읽는 시간이다. 검증 가치는 그 테스트만 잡는 실제 버그와 그 피해다.
15. 화면은 계산된 결과(금액, 날짜, 걸러지고 정렬된 행, 상태 코드, 리다이렉트 위치, 422에서 다시 채운 입력값)로 확인한다. 헤딩 id, 클래스, 폼 id, 속성 순서, 컬럼 순서, 정적 문구를 고정하지 않는다. 같은 응답의 다른 확인에 이미 함축된 마크업을 따로 확인하지 않는다.
16. 지운 기능이 없는지, 등록하지 않은 경로가 404인지, 코드에 없는 분기를 확인하지 않는다. 도달할 수 없는 분기와 DB 제약조건이 이미 막는 결과도 테스트하지 않는다.
17. table-driven 테스트의 행은 분기나 경계마다 하나다. 같은 분기를 지나는 행을 더하지 않는다. 각 행은 자기가 가리키는 분기를 지우면 실패해야 한다. 공통 헬퍼(`web.ParseChoice`)와 표준 라이브러리(`time.Parse`, `http.ServeMux`)의 동작을 호출하는 곳마다 다시 확인하지 않는다.
18. 입력 검증 규칙은 단위 테스트가 케이스별로 확인한다. HTTP 통합 테스트는 handler의 실패 분기마다 잘못된 입력 하나로 422를 확인한다. 한 동작을 두 계층이 확인하게 되면 한쪽을 지운다(9.3).
