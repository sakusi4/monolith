...
func TestAssetQuery_ListURL(t *testing.T) {
	...
	if got, want := q.listURL(month), "/finance/assets?direction=asc&month=2026-09&order=name&type=loan"; got != want {
		t.Errorf("listURL() = %q, want %q", got, want)
	}
	if got, want := q.itemURL(month, "items/7/edit"), "/finance/assets/2026-09/items/7/edit?direction=asc&order=name&type=loan"; got != want {
